...
CON
  ...
  'pix_ntsc      = 11 'debug no border
  'pix_ntsc       = 20  'debug 128 pixel length
...
' Field
'
                        ' Special case: send the 'prepare scanline 0' in advance
                        mov next_line, #0
                        wrlong next_line, request_scanline
...
:vert   if_z            xor     interlace,#1            'interlace skip?
        if_z            tjz     interlace,#:skip
                        ...
                        wrlong next_line, request_scanline  ' Send request for this scanline
                        add next_line, #1

                        call    #hsync                  'do hsync

                        'debug  comment out bottom 3 lines to remove border on left
...
' 44 pulses / cycle (44/3016 * 52.6us = 0.76us, 0.76us * 20MIPS = 15 instructions/packet)
                        ...
                        mov vscl, v_scale
                        ' Loop for 256 pixel fetching them off scanline buffer at $5000
                        ' Four pixel at a time  
                        'change to 32 to make 128 pixel length
                        mov x, #64    'total pixels (256) / 4
                        'mov x, #32     'debug  128 total pixels / 4
                        mov screen, display_base
:nextpixel
                        mov tile, phaseflip
                        rdlong pixels, screen
                        add pixels, black               ' Add black $02020202 to all output
                        xor tile, pixels
                        add screen, #4
:code_waitvid           waitvid tile, #%%3210
                        djnz x, #:nextpixel
...
{
                        add next_line, #1
                        wrlong next_line, request_scanline  ' This line is done: send request for the next one
}       
        ...
        if_nz_and_c     add     x,#1
                        ...
                        wrlong next_line, request_scanline ' Send 'end of screen' with scanline 192
