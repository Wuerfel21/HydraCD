...
'' Sprites clip on left and right sides of screen
...
''   use decimals in ASM (See Calc_Sprite routine in JTC_XRacer_101.spin)
...
Draw_Tiled_Font
        ...
        shl temp_adr, #2                 '*4 (each char scanline is 32 bits)
...
:wrt_bg_scan  mov scanbuffer, temp_var3  'write pixel to local sprite buffe
        ...
        jmp #Draw_Sprites                'move to sprite drawing
...
:check_left_brd        
        ...
        mov temp_var1, #255         'grab border offset
        ...
        mov sprite_x__, #0        'start drawing sprite at pixel 0
...
'--check to see if sprite is drawn outside of right screen border
:check_right_brd
...
:no_clip_right
        ...
        and t3, and_mask3 wz         ' mask off all but lower 2 bits(remainder after shift)
...
:grab_bg1 mov temp_var2, scanbuffer   'grab background
        ...
        if_ae add sprite_x_len_, #1
        cmp temp_var1, #5 wz, wc
        if_ae add sprite_x_len_, #1 
...
and_mask3 long          $00000003 'mask all but last 2 bits 
mask_512  long          $200  
char_rom long           $8000
