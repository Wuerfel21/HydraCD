...
' AUTHOR: Colin Phillips, extended/modified by Rainer Blessing
' LAST MODIFIED: 06.24.06
' VERSION 0.3
' Full Duplex - Set for 115,200 bps 8-N-1 protocol
'
' Transfers data between the EEPROM an the PC with the XMODEM protocol.
'
' CHANGES
'
' 0.2
'   - process_snd_xmodem
' 0.3
'   - writes/reads directly to/from EEPROM
...
''
  ...
  XMODEM_packet_no := 0
  XMODEM_mode := 1
  XMODEM_ptr := @read_buffer
  XMODEM_size := size


PUB send_xmodem(size)
'' Receives a file using the XModem Protocol. (Receiver Initiates Handshake)
''
  XMODEM_packet_no := 0
  XMODEM_mode := 1
    ...
    3:' Retrieve 128 Byte Packet Data      
      if(receive_queue(-1, -1)=>CONSTANT(128+2))        ' Packet + 16-bit CRC on queue?
        ...
        if(XMODEM_size=>128)
          ...
          eeprom.Write(XMODEM_ptr, eeprom#LAST_ADDRESS + 1 - XMODEM_size , 128)
        ...
        case u8
          ...
          XMODEM_ACK:
            if(XMODEM_size>0)             
              ...
              send(@u8,1)
              XMODEM_mode := 2
            else ' everything is sent -> sent EOT
              u8 := XMODEM_EOT
             send(@u8, 1)
             XMODEM_mode := 255              
                 
    2:' Send Packet Number, (Packet Number^0xff), 128 bytes packet data.
       retval := 1
       XMODEM_packet_no := (++XMODEM_packet_no)&255       
       send(@XMODEM_packet_no, 1)
       u8 :=(XMODEM_packet_no&255)^255 'XMODEM_packet_no + u8 == 0xff 
       send(@u8, 1)
       XMODEM_mode := 3
    3:' Send 128 Byte Packet Data
      if(XMODEM_size>0)        
        XMODEM_mode := 1       
        retval := 1                  
        if(XMODEM_size=>128)
          eeprom.Read(XMODEM_ptr, eeprom#LAST_ADDRESS + 1 - XMODEM_size, 128)                        
          repeat until eeprom.IsDone
            byte_count := eeprom.GetBytesRemaining
          send(XMODEM_ptr, 128)
          XMODEM_size-=128
          ...
          if(XMODEM_size>0)
            ...
            repeat until eeprom.IsDone
              byte_count := eeprom.GetBytesRemaining
            send(XMODEM_ptr, XMODEM_size)
            XMODEM_size := 128- XMODEM_size
            u8 := 0
            repeat until XMODEM_size-- == 0 
              send(@u8, 1)           ' pad with 0
          
            
        send(@XMODEM_crc, 2) ' todo: create crc        
