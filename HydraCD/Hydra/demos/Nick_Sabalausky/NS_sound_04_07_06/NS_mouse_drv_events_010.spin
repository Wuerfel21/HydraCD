...
''-------------------------------
''v1.0 (2.13.06) (Based on mouse_iso_010)
''- Added event polling system to detect when a mouse button is
...
VAR
  ...
  long  latched_buttons       'NS: A "latched" copy of par_buttons updated every call to event_poll
  long  latched_buttons_prev  'NS: previous state of latched_buttons
...
'' NS: Call this once per frame to detect
''     button pressed and button released events.

  latched_buttons_prev := latched_buttons
  latched_buttons      := par_buttons


PUB event_button_pressed(b) : event

'' NS: Returns true if a particular button has been
''     pressed since the last call to event_poll,
''     or false otherwise.

  event := -((latched_buttons >> b) & (!latched_buttons_prev >> b) & 1)


PUB event_button_released(b) : event

'' NS: Returns true if a particular button has been
''     released since the last call to event_poll,
''     or false otherwise.
