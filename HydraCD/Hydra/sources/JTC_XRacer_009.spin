...
'' string text and sprite rendering routines. Also uses Nick's sound driver v5.0
...
'' opt - places for posible optimisation
''------------------------------------------------------
'' TODO List
''[?] Red computer car looks better blue?
''[ ] Replace Mary Had A Little Lamb Music
''[ ] Include computer car in sprite priority check
''[ ] Fix scaled sprites clipping too many pixels on the side
''[ ] Add accel model
CON

  _clkmode = xtal1 + pll8x  ' ?
  _xinfreq = 10_000_000 + 0000 ' Set to 10Mhz and add 5000 to fix crystal imperfection of hydra prototype
  _stack = (105) 'the game will break if there is less than 105 longs free
...
' random stuff
  ...
  byte time_note[2]               'how much time until next note
...
OBJ
  ...
  gfx   : "XRacer_gfx_engine_009.spin"     ' graphics driver
  snd   : "NS_sound_drv_050_11khz_16bit.spin" 'Sound driver
PUB start      | i,ii,iii
  DIRA[0] := 1
  outa[0] := 0
...
' Reset everything for a new game
  ...
  'music_vol[1]:=100
  'music_adr[1]:=@Music2
  Music_Toggle:=0 'toggle music off  
  'text for HUD
  ...
  New_Comp_Car(CONSTANT(7+sprite_buff_off))
 ...
 'engine sound
  snd.PlaySoundFM(0, snd#SHAPE_NOISE, 300,CONSTANT(snd#DURATION_INFINITE),100, $FFFF_FFFF)
  snd.PlaySoundFM(1, snd#SHAPE_TRIANGLE, 300,CONSTANT(snd#DURATION_INFINITE),50, $2457_9DEF)
   ...
   ' Read both gamepad
    ...
    ' handle track events         
     ...
     if(C_Car_Dist[0]>240) 'if car is on screen
      ...
      snd.SetFreq(3, (C_Car_Dist[0]>>1)+50)      
     else  'turn off computer car sound
      snd.SetFreq(2, 1)
      snd.SetFreq(3, 1)
     'engine sound for player car        
     snd.SetFreq(0, (car_speed*30)+300)
     snd.SetFreq(1, (car_speed*20))
     if(Music_Toggle)
        PlaySong(0) 'music
...
'handle the player car crashing
      ...
      if(sprite_x[1]+sprite_x_len[1]-10 > _sprite_x_l)
       ...
       if(n==2)
        ...
        if(crash_value<>2)'if we hit a car already, don't change speed 
         ...
         car_speed:=(car_speed*10)/ 14 'slow down a little bit
...
' sprites
  ...
  temp_car_y:=sprite_y[CONSTANT(7+sprite_buff_off)]+sprite_y_len[CONSTANT(7+sprite_buff_off)]
  repeat nn from 6 to 3  'work backwards since the last sprite is drawn first
 {
   if(nn==2) 'if we are on the last one and we haven't gotten to the car yet
    if(temp_car_y<>255)
     temp_car_y:=0 
   if(temp_car_y<(sprite_y[n]+sprite_y_len[n]))
    n2:=CONSTANT(7+sprite_buff_off)
...
'objects 0-nothing, 1-tree, 2-sign, 3-cart
  ...
  repeat while Road_Obj<1  'make sure road is always populated
   Road_Obj:=(?rand & 03)   'grab road side object
   Road_Obj<<=4           'shift object
   Road_Obj+=(?rand & 03) 'grab other roadside object
   Random_Var[2]:=Road_Obj
'0-no change shortest,1-short, 2-medium, 3-long
'4-enter small left turn, 5-enter full left turn
'6-enter small right turn, 7-enter full right turn
'8-exit turn, 99-end of track, reset
  ...
  elseif(Random_Var[0]==1) 'entered a turn
   ...
   Random_Var[1]:=(?rand & $03) 'grab length of turn
   track_len:=(Random_Var[1]+1) <<1'grab value of track traveled   
  else
   Random_Var[0]:=0 'move out of turn
   Random_Var[1]:=8 'exit turn
   track_len:=2 'grab value of track traveled

  if(Random_Var[3]>80) 'length around track
   if(Random_Var[1]==0) 'make sure we aren't in a turn
     Random_Var[3]-=81 'reset track
...
'0-straight, 1-enter left turn, 2-exit left turn, 3-enter right turn, 4-exit right turn, 99-reset
    ...
    'handle turn force
    ' (max speed = 10)*(max turn = 77)= max turn force is 770
    ' (max speed w/o skidding = 6) * (max turn = 77) = turn force w/o losing ground is 462
    if(curve_depth<78) 'make sure we are in a turn
     nn:=road_scanlines-curve_depth 'grab degree of turn
     'grab force of turn, first we find the turn force (curve_depth*car_speed)
     n:=(nn*car_speed)
     'next we multiply that by how much the max value car car can turn *car_turn)
     if(car_turn>0)
      n*=car_turn
     else  'or double that if we aren't turning, double the force
      n*=car_speed<<1
     'then divide it by the min amount of turn force without losing ground / 462
     n/= 462   
     'handle the skidding
     if(car_turn>0) 
      if(n>car_turn) 'if we lose ground in the turn
       snd.PlaySoundFM(4, snd#SHAPE_SINE, 800, 500,200, $2457_9DEF) 'skidding
     if(curve_direction==0) 'left hand turn
        road_x_off+=n   'force of push depending on depth of turn
...
PUB Calc_EE_Turn | kk, temps1, temp_scan_l, temp_road_val      
   ...
   temp_road_val+=car_speed>>1   'move object forward with speed
...
PUB Calc_Road_Sprites(sp_number) | kk, temps1,temps2, temp_scan_l, temp_road_val
   ...
   if(sp_number> CONSTANT(6+sprite_buff_off))   
...
'  time_note[4] 'how much time until next note
   ...
   if(time_note[num]<1)     'if it is at zero, grab next note
    ...
    s_length:=WORD[@T_Notes+(S_Note_Time<<1)]     
    'load note freq
    s_freq:=word[@M_Notes+(S_Note<<1)]
    'if note is not a rest, play note
    if(s_freq>0)
     if(s_freq<1000) 'normal note
'      snd.PlaySoundFM(num, snd#SHAPE_SQUARE, s_freq, s_length,music_vol[num], $2457_9DEF)
      snd.PlaySoundFM(num+6, snd#SHAPE_SQUARE, s_freq, s_length,music_vol[num], $FFFF_FFFF)
     else
      s_freq-=1000  'beat
...
T_Notes 'length of notes; whole note, half note, quarter note, eigth note
word 8_000, 4_000, 2_000, 1_000
...
byte 3,2,8, 2,2,8, 1,2,8, 2,2,8, 3,2,8, 3,2,8, 3,1,8, 0,0,8
byte 2,2,8, 2,2,8, 2,1,8, 0,0,8, 3,2,8, 5,2,8, 5,1,8, 0,0,8
byte 3,2,8, 2,2,8, 1,2,8, 2,2,8, 3,2,8, 3,2,8, 3,1,8, 0,0,8 
byte 2,2,8, 2,2,8, 3,2,8, 2,2,8, 1,1,8, 0,0,8, 0,0,8, 0,0,8, 255
'debug
...
racer_bg000 long $2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B,$2B2B2B2B
        ...
        long $00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$02000000,$05050503,$05050505,$05050505,$03050505,$00000002,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000
        long $00000000,$03000000,$03030303,$03030303,$03030303,$03030303,$00000003,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$00000000,$03020000,$00010101,$00000000,$00000000,$01010100,$00000203,$00000000,$00000000,$BBC90000,$BBBBBBBB,$BBBBBBBB,$BBBBBBBB,$BBBBBBBB,$0000C9BB,$00000000
        long $00000000,$02030000,$01010101,$00000000,$00000000,$01010101,$00000302,$00000000,$00000000,$CACABB00,$C9C9C9C9,$C9C9C9C9,$C9C9C9C9,$C9C9C9C9,$00BBCACA,$00000000,$00000000,$01030000,$01010101,$00000001,$01000000,$01010101,$00000301,$00000000,$00000000,$C9CACABB,$00000000,$00000000,$00000000,$00000000,$BBCACAC9,$00000000
        long $00000000,$01020200,$01010101,$01010101,$01010101,$01010101,$00020201,$00000000,$00000000,$00C9CACA,$01010101,$00000000,$00000000,$01010101,$CACAC900,$00000000,$00000000,$01010302,$01010101,$01010101,$01010101,$01010101,$02030101,$00000000,$00000000,$0100C9CA,$01010101,$00000001,$01000000,$01010101,$CAC90001,$00000000
        long $00000000,$01010302,$01010101,$01010101,$01010101,$01010101,$02030101,$00000000,$BB000000,$010000CA,$01010101,$00000001,$01000000,$01010101,$CA000001,$000000BB,$00000000,$02020302,$02020202,$02020202,$02020202,$02020202,$02030202,$00000000,$CA000000,$010100CA,$01010101,$00000001,$01000000,$01010101,$CA000101,$000000CA
        long $02000000,$04050503,$04040404,$04040404,$04040404,$04040404,$03050504,$00000002,$CA000000,$010100CA,$01010101,$00010101,$01010100,$01010101,$CA000101,$000000CA,$02000000,$05040505,$05050505,$05050505,$05050505,$05050505,$05050405,$00000002,$CABBBB00,$010101CA,$01010101,$01010101,$01010101,$01010101,$CA010101,$00BBBBCA
        long $05050200,$05050305,$05050505,$05050505,$05050505,$05050505,$05030505,$00020505,$CA0505BB,$C9C9C9C9,$BBBBC9C9,$BBBBBBBB,$BBBBBBBB,$C9C9BBBB,$C9C9C9C9,$BB0505CA,$05040402,$05050305,$05050505,$05050505,$05050505,$05050505,$05030505,$02040405,$01BBBBCA,$CABBC905,$CACACACA,$BBCACACA,$CACACABB,$CACACACA,$05C9BBCA,$CABBBB01
        long $04C9C903,$05050305,$05050505,$05050505,$05050505,$05050505,$05030505,$03C9C904,$C901BB00,$C9C905C9,$C9C9C9CA,$C9C9C9C9,$C9C9C9C9,$CAC9C9C9,$C905C9C9,$00BB01C9,$04C9C903,$04040304,$04040404,$04040404,$04040404,$04040404,$04030404,$03C9C904,$BA01CA00,$0505C9C9,$BBC9BB01,$05050503,$03050505,$01BBC9BB,$C9C90505,$00CA01BA
        long $04BAC903,$05050503,$05050505,$05050505,$05050505,$05050505,$03050505,$03C9BA04,$BA01CA00,$01C9BABA,$BBBBBBC9,$05050303,$03030505,$C9BBBBBB,$BABAC901,$00CA01BA,$04BABA03,$05050503,$05050505,$04050505,$05050504,$05050505,$03050505,$03BABA04,$01C9CABB,$BB010101,$BBBBC9C9,$03030303,$03030303,$C9C9BBBB,$010101BB,$BBCAC901
        long $04AABAC9,$05050503,$05050505,$04050505,$05050504,$05050505,$03050505,$C9BAAA04,$BBBBCACA,$C9BBBBBB,$BBBBC9C9,$BBBBBBBB,$BBBBBBBB,$C9C9BBBB,$BBBBBBC9,$CACABBBB,$04AABABA,$05050503,$05050505,$05050505,$05050505,$05050505,$03050505,$BABAAA04,$BBBBCACA,$C9BBBBBB,$CACACACA,$CACACACA,$CACACACA,$CACACACA,$BBBBBBC9,$CACABBBB
        long $04AABAAA,$04040203,$04040404,$04040404,$04040404,$04040404,$03020404,$AABAAA04,$BBCACACA,$BBBBBBBB,$C9C9C9C9,$C9C9C9C9,$C9C9C9C9,$C9C9C9C9,$BBBBBBBB,$CACACABB,$03040403,$05050402,$04050505,$03030303,$03030303,$05050504,$02040505,$03040403,$C9C9C9CA,$C9C9C9C9,$C9C9C9C9,$C9C9C9C9,$C9C9C9C9,$C9C9C9C9,$C9C9C9C9,$CAC9C9C9
        long $02040402,$04040404,$03040404,$04040404,$04040404,$04040403,$04040404,$02040402,$C9C9C9CA,$C9C9C9C9,$C9C9C9C9,$C9C9C9C9,$C9C9C9C9,$C9C9C9C9,$C9C9C9C9,$CAC9C9C9,$03030303,$03030303,$02030303,$04040404,$04040404,$03030302,$03030303,$03030303,$C9C9CACA,$C9C9C9C9,$CACACAC9,$CACACACA,$CACACACA,$C9CACACA,$C9C9C9C9,$CACAC9C9
        long $02020202,$02020202,$01020202,$04040404,$04040404,$02020201,$02020202,$02020202,$C9CACACA,$CACACAC9,$CACACACA,$CACACACA,$CACACACA,$CACACACA,$C9CACACA,$CACACAC9,$02020201,$02020202,$02020202,$01010101,$01010101,$02020202,$02020202,$01020202,$C9CACACA,$050503BB,$02030505,$02020202,$02020202,$05050302,$BB030505,$CACACAC9
