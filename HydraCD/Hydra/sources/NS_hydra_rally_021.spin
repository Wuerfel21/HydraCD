...
VAR
  ...
  ' sound
  long curr_freq   'current frequency of engine sound
  long target_freq
...
'  longmove(@colors, @color_data, 12)
  ...
  curr_freq := DROPIN_FREQ_START
  target_freq := DROPIN_FREQ_END
  ...
  repeat while TRUE
    ...
    if(player_img_ptr == @player_dead_pix) 'player dead
      ...
      if(player_y > PLAYER_START_Y)
        ...
        if(player_y == CONSTANT(SCREEN_BOTTOM-14) )' and curr_freq == 0)
      ...
      if((nes_buttons <> $FFFF and nes_buttons & NES_RIGHT) or (-key.keystate(KB_RIGHT_ARROW)))
        player_x += 1
        if(is_turning == false)
          target_freq := TURNING_FREQ
          is_turning := true
      elseif((nes_buttons <> $FFFF and nes_buttons & NES_LEFT) or (-key.keystate(KB_LEFT_ARROW)))
        player_x -= 1
        if(is_turning == false)
          target_freq := TURNING_FREQ
          is_turning := true
    ...
    if(curr_freq <> target_freq)
      if(target_freq > curr_freq)
        curr_freq += DROPIN_FREQ_DELTA
      else
        curr_freq -= DROPIN_FREQ_DELTA

      snd.SetFreq(0, curr_freq)

  ' end of main loop

' //////////////////////////////////////////////////////////////////

PUB DrawTrack(portion) | i, x1, y1, x2, y2, x3, y3, x4, y4, wall_segment, next_wall_segment, left_line_length, right_line_length
  ...
  repeat i from 0 to NUM_WALL_SEGMENTS-2
    ...
    x1 := wall_segment_x[next_wall_segment] - (player_x~>2)
    y1 -= WALL_SEGMENT_HEIGHT

    x2 := wall_segment_x[wall_segment] - (player_x~>2)
    ...
    if(portion == 0)
      ...
      gr.colorwidth(TRACK_OUTLINE_COLOR, $12)
      ...
      ' draw gates
      gr.colorwidth(TRACK_OUTLINE_COLOR, $10)
      gr.plot(x4,      y4)
      gr.line(x4 - 3  + (x4 ~> 3), y4 + 22 + (y4 ~> 2))
      gr.line(x4 - 15 + (x4 ~> 3), y4 + 27 + (y4 ~> 2) + (y4 ~> 5))

      gr.line(x1 + 15 + (x1 ~> 3), y1 + 27 + (y1 ~> 2) + (y1 ~> 5))
      gr.line(x1 + 3  + (x1 ~> 3), y1 + 22 + (y1 ~> 2))
      gr.line(x1,      y1)
