' /////////////////////////////////////////////////////////////////////////////
' TITLE: caterpillar_demo_001.spin - Framed animation demo.
'
' DESCRIPTION: This demo shows off framed animation along with motion compensation
' for the animated frames. The animation is of a caterpillar that is 32x16 pixels
' thus two sprites are drawn next to each other to make up the larger 32x16 sprite. 
' Also, demo uses sound driver to play a sound when the caterpillar moves to further
...
' COMMENTS:
'
' CONTROLS: Gamepad, right left scrolls image back and forth while animation occurs.
...
long cat_x, cat_y                                       ' position of caterpiller
long cat_frame_index                                    ' current index into animation tables
long cat_num_frames                                     ' total number of frames in animation
long cat_anim_frame                                     ' actual animation frame to lookup in sprite table
long cat_animation_counter                              ' animation counter
...
tile_data:      "caterpillar_tile_data.spin" 

snd:            "NS_sound_drv_050_22khz_16bit.spin"      'Sound driver
snd_data:       "caterpillar_11_8.spin"                  'PCM Sound Effect "Hydra"
...
tile_map_sprite_cntrl_parm    := $02_00 ' 2 sprites, tile map set to 0=16 tiles wide, 1=32 tiles, 2=64 tiles, etc.
tile_sprite_tbl_base_ptr_parm := @sprite_tbl[0] ' point to sprite data table 
tile_status_bits_parm         := 0

' initial position of caterpiller
cat_x := $10
cat_y := $80

' reset current animation frame index
cat_frame_index := 1

' look up number of frames of animation, always in element 0
cat_num_frames := animation_frames[0]

' extract first frame of animation
cat_anim_frame := animation_frames[ cat_frame_index ]

' enable/initialize a sprite
sprite_tbl[0] := cat_y << 24 | cat_x << 16 | $01         ' sprite 0 state: y=xx, x=$xx, z=$xx, enabled/disabled
sprite_tbl[1] := tile_data.sprite_bitmaps + (cat_anim_frame*256)                ' sprite 0 bitmap ptr

sprite_tbl[2] := cat_y << 24 | (cat_x+$10) << 16 | $01   ' sprite 1 state: y=xx, x=$xx, z=$xx, enabled/disabled
sprite_tbl[3] := tile_data.sprite_bitmaps + (cat_anim_frame*256)+128            ' sprite 1 bitmap ptr
...
repeat while 1
  ...
  ' animate caterpiller if its time to do so
  ...
  ' draw caterpiller, take into consideration scroll_x, that is "world view" position since sprites and tiles move indepedantly
  sprite_tbl[0] := cat_y << 24 | (cat_x-scroll_x*16) << 16 | $01        ' sprite 0 state: y=xx, x=$xx, z=$xx, enabled/disabled
  sprite_tbl[1] := tile_data.sprite_bitmaps + (cat_anim_frame*256)      ' sprite 0 bitmap ptr

  sprite_tbl[2] := cat_y << 24 | (cat_x-scroll_x*16+$10) << 16 | $01    ' sprite 1 state: y=xx, x=$xx, z=$xx, enabled/disabled
  sprite_tbl[3] := tile_data.sprite_bitmaps + (cat_anim_frame*256)+128  ' sprite 1 bitmap ptr
