...
CON
   ...
   PALETTE_STARTUP     = %0000_0_111__0000_0_111__0000_0_111__0010_1_011
'   PALETTE_STARTUP     = %0000_0_111__0000_0_111__0000_0_111__0010_1_100   
...
OBJ    ' instantiate all objects
   ...
   TV   : "tv_drv_010.spin"         
   FUNC : "RGW_HTBasic_Support_010.spin"
...
PUB start | lIndex, lRetState, lPtr, lX, lY, l0 
   ...
   ' Start TV cog.
   long[@tvScreen] := @Tiles
   long[@tvColors] := @Palette
   tv.start(@tvParams)

   ' Setup the 4 colors for Palette[0] (main game palette).
   Palette[0] := PALETTE_STARTUP  
   
   ' Init tile screen map to bit buffer, init all tiles to use palette 0.
   repeat lX from 0 to X_TILES - 1
      repeat lY from 0 to Y_TILES - 1
         Tiles[lY * X_TILES + lX] := CONSTANT(onscreen_buffer >> 6) + lY + lX * Y_TILES      

   ' Start the Helper cog
   pParams:=FUNC.Start(@BUFFER, @TB_TOKENS)
   FUNC.Setup(onscreen_buffer, 0, 0, @Debugger,0,0,0,0,0,0)
...
F_RND                   call #EXPR              ' Pick up the limit
                        mov D4, FUNC_RND
                        call #RND_FUNC
                        jmp #POP_RETN
...
TEST_VARS
              ...
              if_nz     jmp #TEST_VAR           ' Jmp if not the '@' array
                        ...
                        sub SP, #4              ' PARN uses this routine so 
                        wrlong TEST_VAR_RET, SP ' we need to save the return address           
                        call #PARN              ' (EXPR) as it's index
                        shl D0, #2 wc           ' long boundry
             ...
             if_be      jmp #ERR_OOM            ' Error if out of memory
                        ...
                        mov Temp6, Temp
                        rdlong TEST_VAR_RET, SP ' Restore the return address
                        add SP, #4
