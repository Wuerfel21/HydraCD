' //////////////////////////////////////////////////////////////////////
' Simple scaling demo with translation, scaling and double buffering
' also, renders copies of objects to get a fleet of them !
...
' A to make object larger
' B to make object smaller
' //////////////////////////////////////////////////////////////////////
...
CON
  ...
  PSCALE            = 1  ' can set to 1 now since we have on-the-fly scaling
...
VAR
  ...
  ' scale of ship, note always is an integer
  long scale
...
PUB start | i, dx, dy, x, y
  ...
  repeat while TRUE
    ...
    ' update real-time scaling factor
    if (nes_buttons & NES_A)
      scale++
      waitcnt (cnt + 50_000)    ' use real time counter for delay, wait "now + 50,000 clocks" 
    ...
    ' draw fleet of 3x3 ships
    repeat y from 0 to 2
      repeat x from 0 to 2
        ' color them, and seperate them (try to proportionately seperate them?) Hint: replace the "40" by k*scale
        ' but then notice that they don't scale from the center? hmmm, try moving the "orgin" as the image scales :)        
        Draw_Polygon3(@fighter_poly, 12, 1+(x // 3),x*10*scale+ship_x, y*10*scale+ship_y, scale )
