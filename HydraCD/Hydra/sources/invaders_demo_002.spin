' /////////////////////////////////////////////////////////////////////////////
' TITLE: INVADERS_DEMO_002.SPIN - pattern interpreter demo  
'
' DESCRIPTION: This demo shows off using a pattern control language to instruct
' game characters to perform pre-recorded motions and operations, the demo
' is based on a simple opcode, operand instruction set used to create "programs"
' that the alien robots read and execute. The demo also supports a player which
' can move and fire.  
'
' VERSION: 1.0
...
' COMMENTS:
'
' CONTROLS: Gamepad, right left moves player back and forth, gamepad <A> and <B>
' fire weapons
'//////////////////////////////////////////////////////////////////////////////
...
CON
  ...
  BOTS_FORMATION_X0 = 7     ' formation origin in world coords
  BOTS_FORMATION_Y0 = 1
  ...
  NUM_BOT_PATTERNS = 4          ' how many patterns are there
  ...
  ' to add commands you need to add opcode here then code the command in the virtual machine pattern language interpreter
...
word bot_pattern_ptrs[ NUM_BOT_PATTERNS] 
...
' initialize pattern pointer array
bot_pattern_ptrs[0] :=  @bot_pattern_alpha
bot_pattern_ptrs[1] :=  @bot_pattern_beta
bot_pattern_ptrs[2] :=  @bot_pattern_gamma
bot_pattern_ptrs[3] :=  @bot_pattern_delta

' initialize bots all to same startup pattern0 
repeat bot_index from 0 to NUM_BOTS - 1
  ' start bots in 2 x n formation
  bot_state[bot_index]         := BOT_STATE_ALIVE
  bot_type[bot_index]          := bot_index / 2                    
  bot_x[bot_index]             := BOTS_FORMATION_X0 + (bot_index // 2)                       
  bot_y[bot_index]             := BOTS_FORMATION_Y0 + (bot_index / 2)                         
  ...
  bot_pattern_ptr[bot_index]   := bot_pattern_ptrs [ 0 ] ' start all bots with same pattern              
      ...
      case (bot_opcode[bot_index])          
        ...
        PCMD_END:           ' format: {PCMD_END} - ends program.  
          ...
          if (bot_vmstate[bot_index] == BOT_VMSTATE_FETCH)
            ...
            bot_pattern_index[bot_index] := 0      
            bot_counter[bot_index]       := 0           
            bot_pattern_ptr[bot_index]   := bot_pattern_ptrs [Rand_Range(0, NUM_BOT_PATTERNS-1) ]              
...
' bot patterns
bot_pattern_alpha  byte PCMD_IDLE,10, PCMD_RIGHT,6, PCMD_IDLE, 30, PCMD_FIRE, PCMD_LEFT,6, PCMD_IDLE, 30, PCMD_FIRE, PCMD_LEFT,6, PCMD_IDLE,30, PCMD_FIRE, PCMD_RIGHT,6, PCMD_END        
bot_pattern_beta   byte PCMD_DOWN,8, PCMD_UP,8, PCMD_RIGHT,5, PCMD_LEFT,10, PCMD_RIGHT,5, PCMD_END
bot_pattern_gamma  byte PCMD_IDLE,50, PCMD_TRACK,2, PCMD_IDLE,50, PCMD_FIRE, PCMD_END
bot_pattern_delta  byte PCMD_LEFT,4, PCMD_DOWN,1, PCMD_RIGHT,4, PCMD_DOWN,1,PCMD_LEFT,4, PCMD_DOWN,1, PCMD_RIGHT,4, PCMD_DOWN,1,PCMD_LEFT,4, PCMD_DOWN,1, PCMD_RIGHT,4, PCMD_DOWN,1,PCMD_EVADE,4, PCMD_END  
