...
' Video (TV) Related Constants
  ...
  pix_ntsc      = vntsc/256     '(11 pulses per pixel)
...
' Graphics Related Constants
  scanline_buf  = $180          ' Scanline Buffer : $180 - $1c0 (64 longs / 256 bytes)
  obj_buf       = $1c0          ' Object Buffer : $1c0 - $1f0 (48 longs / 192 bytes)
...
entry
                        ...
                        ' setup sprite coords (SPIN HLL would change these)
                        mov     obj_buf+OBJ_OFFSET_X, #55                       ' 50,50
                        mov     obj_buf+OBJ_OFFSET_Y, #16
                        mov     obj_buf+OBJ_OFFSET_W, #(16+3)/4 ' 16x16
                        mov     obj_buf+OBJ_OFFSET_H, #16-1                        
...
next_frame              ' start of new frame
                        ...
                        mov     r0, #obj_total_size                             ' 4*12 LONG's copy loop.
...
' Sprite Build Up /////////////////////////////////////////////////////////////
' 10.9us = 218 instructions can fit here.
...
code_spr_d if_nc        cmp     0, r1                   wc                      ' CARRY = Sprite.H<(Y-Sprite.Y)
      if_c              jmp     #:skip_spr_hline
                        ...
                        mov     r3, obj_buf+OBJ_OFFSET_W                        ' (20) R3 = no. of quad pixels to draw.
...
:code_scanline_d        mov     0,r2                                            ' write to scanline buffer
...
:code_scanline_y        shr     r0, #0                                          ' shift last pixels into place.
                        ...
                        mov     :code_scanline_i, :code_scanline_d              ' copy entire instruction
:code_scanline_i        mov     0,r2                                            ' (5) write to scanline buffer

                                                                                ' Total: (25 + Width*2.75) 'Sprite'
                        
:skip_spr_hline                                                                 ' Total: (7) 'No Sprite'



'                       mov     r2, 15
'                       mov     r1, v_scanline
'                       sub     r1, #50         wc                              ' CARRY = Y<Sprite.Y
'       if_nc           cmp     r2, r1          wc                              ' CARRY = Sprite.H<(Y-Sprite.Y)
'       if_nc           mov     scanline_buf+32, v_tempo
...
code_waitvid            mov     r1, 0
                        add     r1, v_coffset
                        waitvid r1, #%%3210                                     ' 5+ feed thru 4 colors / 4 pixels
