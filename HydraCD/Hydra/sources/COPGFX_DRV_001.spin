...
' Graphics Related Constants
  ...
  #0, h_copgfx_status, h_copgfx_vram, copgfx_bg0_s, copgfx_bg0_h, copgfx_bg0_v
...
'   LONG[@v_scanline_addr+4] := @scanline_memory + 256
 '  LONG[@v_scanline_addr+8] := @scanline_memory + 512
  ...
  repeat i from 0 to CONSTANT(cog_n-1)
    ...
    LONG[@s_bg0_tile] := LONG[copptr+(h_copgfx_vram<<2)]
...
next_vsuframe           
                        ...
                        add     v_scanline_taddr, #0                            ' EXTRA GAP between scanlines. (to help with clipping)
...
' // Scanline Preparation /////////////////////////////////////////////////////
' // for 4+1 cogs, we have 63.5us * 4 (254us) processing, which is 5080 instrs
...
' X+=DX, Y+=DY
                        ...
                        movd    :code_1a, #local_buf
                        mov     m7_p, m7_hp                                     ' current pixel address = horizontal start address.
                        mov     a7, #64                                         ' Do 64*4 pixels
:loop_m7_pixel

' ITTR #0               
                        mov     a0, m7_p                                        ' a0 = YYYYYYYYYYYYyyyy:XXXXXXXXXXXXxxxx
                        shr     a0, #8                                          
                        and     a0, _k_map_wm                                   ' a0 = XXXXXXXX&MAPWIDTH_MASK
                        mov     a1, m7_p
                        shr     a1, _k_map_wsn                                  ' a1 = YYYYYYYY(0*MAPWIDTH_SHIFT)
                        and     a1, _k_map_hm                                   ' a1 = YYYYYYYY&MAPHEIGHT_MASK
                        or      a0, a1                                          ' a0 = YYYYYYYYXXXXXXXX
...
' constants
_k_map_wm               long                    (1<<7)-1
_k_map_wsn              long                    32-8-7
_k_map_hm               long                    ((1<<7)-1)<<7
