...
' use <A>/<B> to rotate the turret cannon, and <START> to fire a projectile,
' <SELECT> to toggle thru information/diagnostic display modes
...
'
' Note: once in a while a projectile (ball) will get "stuck" this is typical
' in physics sims, I leave it to you to fix it! 
'
' //////////////////////////////////////////////////////////////////////
...
VAR
  ...
  byte string_buffer[33]                                  ' buffers strings
...
PUB start | i, j, k, dx, dy, x, y, w, h, sinrot, cosrot, pindex
  ...
  ' INPUT SECTION /////////////////////////////////////////////////////////////
    ...
    ' adjust turret power
    if (gp.button(NES0_UP) and turret_power < 25)
      turret_power++
    
    if (gp.button(NES0_DOWN) and turret_power > 1)
      turret_power--
    ...
    ' print velocity magnitudes and power level    
    if (display_mode == 1 or display_mode == 3)
      gr.textmode(2,1,5,3)
      gr.colorwidth(3,0)

      ItoHex(turret_power, @string_buffer)
      gr.text(SCREEN_WIDTH-82, SCREEN_HEIGHT-16, @string_buffer)
...
' rot              - rotation angle to rotate mesh ANG_0..ANG_360
  ...
  if (rot <> 0)
    ...
    ' rotation angle must be 0, thus use simplified transforms
    repeat vi from 0 to num_verts-1 
      ' transform each point in mesh
