...
VAR
  ...
  byte counter
  'byte scroll_ptr
  'byte scroll_buffer[X_TILES * Y_TILES]
  long box_x, box_y     ' used to draw a box around the playing field.
  long land_x, land_y     ' used for the landscape.
  long world_x, world_y   ' used for scrolling the landscape/world.
  long array_y
...
PUB start | i, dx, dy, x, y
  ...
  repeat counter from array_y to 50 ' loop through the established array
  ...
  repeat while TRUE
    ...
    ' code to check if we reached the ends of the world map.
    if (world_x > SCREEN_WIDTH/2)
      world_x := -SCREEN_WIDTH/2 - 6
    elseif (world_x < -SCREEN_WIDTH/2 - 6)
    ...
    ' draw the terrain
    land_y := -90 ' set initial y value
    repeat counter from array_y to 49 ' loop through the established array
      gr.line(-land_x + counter*10, land_y) ' draw line based on array coords.
      land_y += terrain[counter] ' update y with the coords from the terrain array.
...
' TERRAIN //////////////////////////////////////////////////////////////
terrain                 long 0, 10, -10, 5, -3, 15, -20, 3, -3, 3, -3, 6, -2, 4, -2, 0, 12, -18, 8, -8, 12, -8, 10, -10, 0,0, 10, -10, 5, -3, 15, -20, 3, -3, 3, -3, 6, -2, 4, -2, 0, 12, -18, 8, -8, 12, -8, 10, -10, 2
