...
OBJ
  ...
  snd   : "NS_sound_drv_052_22khz_16bit.spin"      'Sound driver
  ...
  dat_sfx : "NS_hydra_sound_011.spin"              'PCM Sound Effect "Hydra"
...

{
  ...
  'draw snazzy red line
  gr.color(COLOR_KEYBOARD)
  ...
  snd.PlaySoundPCM(CHANNEL_PCM, dat_sfx.ns_hydra_sound, dat_sfx.ns_hydra_sound_end, 255)
  ...
  repeat
    ...
    case key_pressed & $00FF
      ...
      key#KB_SPACE:
        ...
        DrawHydraButton(COLOR_KEYBOARD)
        hydra_highlight_count := HYDRA_BUTTON_HIGHLIGHT_TIME
    ...
    if(keyboard_key_playing <> 0)
      if(not key.keystate(keyboard_key_playing))
        snd.ReleaseSound(CHANNEL_MKEY_KEYBOARD)
    ...
    repeat i from 0 to CONSTANT(MK_NUM_KEYS-1)
      if(mkey_keyboard_map.BYTE[i] == key_pressed & $FF and mkey_keyboard_map.BYTE[i] <> keyboard_key_playing)
        snd.PlaySoundFM(CHANNEL_MKEY_KEYBOARD, snd#SHAPE_SQUARE, key_notes[i], CONSTANT(snd#DURATION_INFINITE | (snd#SAMPLE_RATE>>1)), 255, $2457_9DEF)
    ...
    if(is_shape_changed)
      ...
      if(key_pressed & key#KB_CONTROL_MOD)
        snd.PlaySoundFM(CHANNEL_LONG_TONE, shape, FREQ_MAX, CONSTANT( Round(Float(snd#SAMPLE_RATE) * 0.4)), 255, $3579_ADEF )  'Play for two fifths of a second
        is_stopped := false
      elseif(key_pressed & key#KB_ALT_MOD)
        snd.PlaySoundFM(CHANNEL_SHORT_TONE, shape, FREQ_INIT, CONSTANT( snd#SAMPLE_RATE * 2 ), 255, $3579_ADEF )  'Play for two seconds
        is_stopped := false
      else
    ...
    if(mouse_mkey_playing <> -1 and keyboard_mkey_playing <> -1)
      ...
      if(mouse_mkey_playing < keyboard_mkey_playing)
        ...
        DrawKey(COLOR_KEYBOARD, keyboard_mkey_playing)
      else
        DrawKey(COLOR_KEYBOARD, keyboard_mkey_playing)
        DrawKey(COLOR_MOUSE,    mouse_mkey_playing)
...
PUB HandleMusicKeyboard | i, key_pos, x, y, x_prev, y_prev, is_in_keyboard, was_in_keyboard, left_keyboard
  ...
  if(mouse.event_button_released(0) or left_keyboard)
    snd.ReleaseSound(CHANNEL_MKEY_MOUSE)
    mouse_mkey_playing := -1
    return
      ...
      repeat i from 0 to MK_NUM_BLACKKEYS-1
        ...
        if(x => (key_pos >> 16) and x < (key_pos & $FFFF))
          ...
          if(MK_NUM_WHITEKEYS + i <> mouse_mkey_playing)
            ...
            'play note
            snd.PlaySoundFM(CHANNEL_MKEY_MOUSE, snd#SHAPE_SQUARE, blackkey_notes[i], CONSTANT(snd#DURATION_INFINITE | (snd#SAMPLE_RATE>>1)), 255, $2457_9DEF)
    ...
    repeat i from 0 to MK_NUM_WHITEKEYS-1
      ...
      if(x < (whitekey_pos[i] & $FFFF))
        ...
        if(i <> mouse_mkey_playing)
          snd.PlaySoundFM(CHANNEL_MKEY_MOUSE, snd#SHAPE_SQUARE, whitekey_notes[i], CONSTANT(snd#DURATION_INFINITE | (snd#SAMPLE_RATE>>1)), 255, $2457_9DEF)
