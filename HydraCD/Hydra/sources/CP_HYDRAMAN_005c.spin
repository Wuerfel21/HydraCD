...
' LAST MODIFIED: 1.14.06
' VERSION 0.5c - 4 Player (4 Humans, Bombs/Flames/Punch Gloves Galore! "Hyper Edition")
'
' DESCRIPTION:
' HydraMan - Multiplayer bomb blasting game :-)
' Go to around line 2300 (INITPLAYERS section) if you wish to modify the initial settings for the players including AI.
' Go to around line 33 (CONTROLLER->PLAYER MAPPINGS) if you wish to remap some controller interfaces (there's up to 8 players on the map)
' Go to around line 800 (SOUND ENGINE) if you wish to alter the sound effects.
'
' CONTROLS (default):
...
'
' Notes:
' AI: Basic Lee's Algorithim & Bomb Evasion included, need to add bomb dropping AI, and smarter bomb evasion.
' Mouse: Removed mouse driver.
' Memory: Is pretty low. at 196 Long's left, need to tighten up some code.
'
' //////////////////////////////////////////////////////////////////////
...
' CONTROLLER->PLAYER MAPPINGS
  INPUT_GAMECONTROLLER_0 = 0
  INPUT_GAMECONTROLLER_1 = 1
  INPUT_KEYBOARDWASD = 2
  INPUT_KEYBOARDCURSOR = 3
  INPUT_MOUSE = 4                   ' mouse should be mapped over to 2nd controller since most of us only have 1 controller.
...
long controllerreal[MAX_PLAYERS]    ' real controller input
long controller[MAX_PLAYERS]        ' double buffer
...
byte aibuf[MAP_SIZE]                ' ai buffer / ai map
long gamestate[8]
...
PUB Start | x, y, i, n, dir, anim, frame, mx,my
...
' the top level file
  ...
  g_copptr := @cop_status
  g_copsndptr := @copsnd_len
  g_objptr := @cop_obj
  g_controllerptr := @controllerreal
  g_playerptr := @player
  g_bombptr := @bomb
  g_vramptr := cop_vram
  g_tileptr := cop_tile
  ...
  repeat while TRUE
    ...
    controller[INPUT_GAMECONTROLLER_0]|= i&$ff
    controller[INPUT_GAMECONTROLLER_1]|= (i>>8)&$ff
  ' Keyboard Controllers (two player inputs)
    if(key.keystate($77)) 'W'
      controller[INPUT_KEYBOARDWASD]|=KEY_UP
    if(key.keystate($73)) 'S'
      controller[INPUT_KEYBOARDWASD]|=KEY_DOWN
    if(key.keystate($61)) ' A
      controller[INPUT_KEYBOARDWASD]|=KEY_LEFT
    if(key.keystate($64)) ' D
      controller[INPUT_KEYBOARDWASD]|=KEY_RIGHT
    if(key.keystate($F0)) ' Shift-L
      controller[INPUT_KEYBOARDWASD]|=KEY_B
    if(key.keystate($F2)) ' Ctrl-L
      controller[INPUT_KEYBOARDWASD]|=KEY_A

    if(key.keystate($C2))
      controller[INPUT_KEYBOARDCURSOR]|=KEY_UP
    if(key.keystate($C3))
      controller[INPUT_KEYBOARDCURSOR]|=KEY_DOWN
    if(key.keystate($C0))
      controller[INPUT_KEYBOARDCURSOR]|=KEY_LEFT
    if(key.keystate($C1))
      controller[INPUT_KEYBOARDCURSOR]|=KEY_RIGHT
    if(key.keystate($F1)) ' Shift-R
      controller[INPUT_KEYBOARDCURSOR]|=KEY_B
    if(key.keystate($F3)) ' Ctrl-R
      controller[INPUT_KEYBOARDCURSOR]|=KEY_A

    {
  ' Mouse Controller (one input)    
    mx := mouse.delta_x
    my := -mouse.delta_y

    if(mx>0)
      controller[INPUT_MOUSE]|=KEY_RIGHT
    if(mx<0)
      controller[INPUT_MOUSE]|=KEY_LEFT
    if(my>0)
      controller[INPUT_MOUSE]|=KEY_DOWN
    if(my<0)
      controller[INPUT_MOUSE]|=KEY_UP
    if(mouse.buttons&1)    ' Left
      controller[INPUT_MOUSE]|=KEY_A
    if(mouse.buttons&2)    ' Right
      controller[INPUT_MOUSE]|=KEY_B
    }
    
    ' update controllers at once.
    repeat i from 0 to MAX_PLAYERS-1
      controllerreal[i] := controller[i]
      controller[i] := 0
    ...
    if((frame&63)==0)
      copsnd_cnt := 0
      copsnd_volume := 128<<8
      copsnd_freq := 511                                ' 0..8192
      copsnd_phase := 0
      copsnd_venv := $0369cff8
      copsnd_fenv := $0
      copsnd_tick := 0
      copsnd_len := 11025                               ' Always set last

    if(((frame+8)&15)==0)
      copsnd_cnt := 0
      copsnd_volume := 128<<8
      copsnd_freq := 384                                ' 0..8192
      copsnd_phase := 0
      copsnd_venv := $36cc963   '$048cc840 '$0369cff8
      copsnd_fenv := $0
      copsnd_tick := 0
      copsnd_len := 11025>>2                            ' Always set last
             }
...
g_frame                 LONG                    $0
g_bombidx               LONG                    $0
g_sndreq                LONG                    $0      ' we'll use this to handle the generate sound sample request. - so 1 sample issued per frame.
g_aibuf                 LONG                    $0
...
' process...
                        ...
                        ' === Execute Block of Code "PROCESSAI"
                        mov    __loader_page, #(_memstart+@PROCESSAI_START)>>9
                        shl    __loader_page, #9                     
                        or     __loader_page, #(_memstart+@PROCESSAI_START) & $1FF           
                        mov    __loader_size, #(PROCESSAI_END-PROCESSAI_START)
                        mov    __loader_jmp, #PROCESSAI_START
                        jmpret __loader_ret,#__loader_call                        
                        ...
                        ' === Execute Block of Code "PROCESSSND"
                        mov    __loader_page, #(_memstart+@PROCESSSND_START)>>9
                        shl    __loader_page, #9                     
                        or     __loader_page, #(_memstart+@PROCESSSND_START) & $1FF          
                        mov    __loader_size, #(PROCESSSND_END-PROCESSSND_START)
                        mov    __loader_jmp, #PROCESSSND_START
                        jmpret __loader_ret,#__loader_call
...
' draw...
                        ...
                        {
                        ' === Execute Block of Code "DRAWSTATS"
                        mov    __loader_page, #(_memstart+@DRAWSTATS_START)>>9
                        shl    __loader_page, #9                     
                        or     __loader_page, #(_memstart+@DRAWSTATS_START)           
                        mov    __loader_size, #(DRAWSTATS_END-DRAWSTATS_START)
                        mov    __loader_jmp, #DRAWSTATS_START
                        jmpret __loader_ret,#__loader_call
                        }
...
PROCESSSND_START
        ...
        if_z            jmp #:nosound
                        ...
                        rdlong :copsndptr, :copsndptr   ' get address of Sound Co Processor parameters.
...
:k_d0                   long                    1<<9
:sndreq                 long                    _memstart+@g_sndreq
:copsndptr              long                    _memstart+@g_copsndptr
...
:sample_lut
                        ...
                        long                    0       ' CNT
                        long                    128<<8  ' VOL
                        long                    FRQ_WHITENOISE | 0 '128 ' FREQ
                        long                    2048    ' PHASE
                        long                    $369cffff'$48cffc84 '$f0f0f0f ' VOL ENV.
                        long                    $0000000 ' FREQ ENV.
                        long                    0       ' TICK
                        ' #4 PUNCH BOMB
                        long                    11025/16 ' LENGTH
                        long                    0       ' CNT
                        long                    128<<8  ' VOL
                        long                    FRQ_WHITENOISE | 128 ' FREQ
                        long                    0       ' PHASE
                        long                    $6cfdc6 ' VOL ENV.
                        long                    $0000000 ' FREQ ENV.
                        long                    0       ' TICK
...
:not_bomb
                        ' Flame Check /////////////////////
                        rdbyte r2, r7           wz ' Flame map. Flame?
        if_nz           cmp    r2, #255         wz ' ignore predicted flame?
        if_z            jmp    #:not_flame
                        ' Make flames tick down to 0 then turn off.
                        rdbyte r2, r1           wz
                        min    :max_flame, r2   ' update max attenuation for effect.
        if_nz           sub    r2, #1
...
:not_brick
                        ...
                        rdlong :r_copptr, :r_copptr
                        add    :r_copptr, #h_cop_debug*4
                        shl    :max_flame, #24+3
                        wrlong :max_flame, :r_copptr
...
:randadd                long                    $1f58932d
:max_flame              long                    $0
:r_copptr               long                    _memstart+@g_copptr
...
:loop
        ...
        if_nz           add    r2, #:lut_tmap
                        ...
                        cmp    r2, #255         wz      ' predicted flame
        if_z            mov    r2, #0
        if_z            wrbyte r2, r3 ' fmap[f] = 0     ' wipe out predicted flame.
                        cmp    r2, #0           wz
                        
'                       cmp    r2, #0           wz
'       if_nz           cmp    r2, #255         wz      ' predicted flame
        if_z            jmp    #:noflame
...
'       if_z            jmpret :explode_ret, #:explode
                        ...
                        jmpret :explode_ret, #:explode  ' always call this - for the predicted flames.
:next                              
...
'       if_nz           jmp    #:skip_bombland
        ...
        if_nc           jmp    #:skip_bombland          ' still on the initial big bounce?
                        ...
                        rdbyte r6, :t1                  wz                      ' no flame?
        if_nz           cmp    r6, #255                 wz

        if_nz           jmp    #:ok_solid       ' real flame exists - treat as solid.

                        ' real flame doesnt exist, check if cell is solid.
...
' NZ - Predict
        ...
        if_nz           jmp    #:predict
                        ...
                        mov    r2, #SFX_EXPLOSION ' Explosion SFX
                        wrlong r2, :sndreq
...
:floop
        ...
        if_z_and_nc     jmp    #:continueflame
                        ...
                        cmp    :midsect, #255   wz      ' predicted. do a silent abort
        if_z            jmp    #:endfloop
...
:continueflame
                        ...
                        rdbyte r6, :fp          wz
        if_nz           cmp    r6, #255         wz  ' 0 or 255 we can write over, but not a real flame.
        if_nz           jmp    #:nowriteover
        
                        cmp    :len, #1         wz
        if_z            wrbyte :endsect, :fp
        if_nz           wrbyte :midsect, :fp
        
                        cmp    :midsect, #255   wz  ' dont write extended data if this is a predicted flame.
        if_nz           wrbyte r2, :ep
...
' 1. generate predicted flame map - (use logical map for bombs & ground's + flame map)
' 2. run to closest safe square - (lee's algorithm)

                        rdlong :controllerptr, :controllerptr
                        rdlong :playerptr, :playerptr
                        rdlong :aibuf, :aibuf
                        rdlong :lmap, :lmap
                        rdlong :fmap, :fmap
                        mov    r1, #0
...
:loop
                        mov    r0, :playerptr
                        rdlong r2, r0           wz      ' PLAYER_STATUS!=0
        if_z            jmp    #:skip_ai
                        'mov   r0, :playerptr
                        add    r0, #PLAYER_AI*4
                        rdlong :ai, r0          wz      ' PLAYER_AI>0
        if_z            jmp    #:skip_ai

                        ' :x = player[r1].x+8 >> 4 (cell coords)
                        ' :y = player[r1].y+8 >> 4
                        mov    r0, :playerptr
                        add    r0, #PLAYER_X*4
                        rdlong :x, r0
                        ...
                        shr    :y, #4
                        ' :tx , :ty target cell.
                        mov    :tx, #0          ' 0 = no specific target.
                        mov    :ty, #0

                        ' FIND CLOSEST SAFE SQUARE
                        jmpret :lee_algo_ret, #:lee_algo
                        mov    r0, :playerptr
                        add    r0, #PLAYER_BUTTON*4
                        wrlong :lee_dir, r0
                        
'                       cmp    :lee_dir, #0     wz
'       if_nz           jmp    :skip_itemfind
'
'                       ' FIND CLOSEST ITEM
'                       jmpret :lee_algo_ret, #:lee_algo
'                       mov    r0, :playerptr
...
:skip_ai
                        ...
                        add    :playerptr, #PLAYER_SIZE*4
                        add    :controllerptr, #4
                        add    r1, #1
                        cmp    r1, #MAX_PLAYERS wz
        if_nz           jmp    #:loop
...
:lee_algo
                        ...
                        ' 1) set aibuf to traversable (0) / untraversable (255) (based on lmap's ground tiles)
                        ...
                        mov    r4, #MAP_SIZE
:lee_clearloop
                        mov    r5, #255         ' default untraversable
                        rdbyte r6, r2
                        shr    r6, #5
        
                        cmp    r6, #MAP_GROUND>>5       wz
        if_z            mov    r5, #0           ' traversable
        
        if_z            rdbyte r6, r7          wz ' if traversable & no flame
        if_z            mov    r5, #1             ' safe: plant seed.

'                       cmp    r6, #MAP_ITEM>>5 wz
'       if_z            mov    r5, #1           ' plant a seed

                        wrbyte r5, r3           ' *aimap++ = (*lmap++ >> 5) == (MAP_GROUND >> 5) ? 255 : 0
                        
                        add    r2, #1
                        add    r3, #1
                        add    r7, #1
                        djnz   r4, #:lee_clearloop

                        ' 2a) plant seed (1) where destination is (:tx,:ty) - we'll run in reverse (dest->source).
                        ...
                        'add   r3, :aibuf
                        'mov   r5, #1
                        'wrbyte r5, r3          ' aibuf[ty*width+tx] = 1
                        
                        ' 2b) plant stop (254) where source is (:x,:y)
                        mov    r3, :y
                        shl    r3, #MAP_WIDTHS
                        add    r3, :x
                        add    r3, :aibuf
        ...
        if_z            jmp    #:lee_done
                        mov    r5, #254
                        wrbyte r5, r3           ' aibuf[y*width+x] = 254
                        
                        ' 3) grow the seed "lava/creeping vine routine" 0, 1 to MAP_WIDTH-1, MAP_HEIGHT-2
...
' outer loop //////////////////////////////////////////////////////////////////
                        ...
                        mov    :safe, #10               ' 10 steps/squares max.
...
'                       mov    r3, :lmap                ' DEBUG
                        add    r2, #MAP_WIDTH
'                       add    r3, #MAP_WIDTH           ' DEBUG
                        mov    r4, #MAP_SIZE-MAP_WIDTH*2
:lee_growloop
                        rdbyte r5, r2                      
                        cmp    r5, r6           wz         ' r5 == r6
        if_nz           jmp    #:lee_growcontinue1         ' [BUG] 'Expression is too complex' what is this bug??? (remove the '1' from the label and i get it)
                        
                        ' stamp down r7 value's. around value r6
                        sub    r2, #MAP_WIDTH
'                       sub    r3, #MAP_WIDTH           ' DEBUG
                        rdbyte :cell_up, r2             wz
        if_z            wrbyte r7, r2                      ' aimap(x,y+1) = r7
'       if_z            wrbyte :t, r3                   ' DEBUG
        
                        add    r2, #MAP_WIDTH-1
'                       add    r3, #MAP_WIDTH-1         ' DEBUG
                        rdbyte :cell_left, r2           wz
        if_z            wrbyte r7, r2                      ' aimap(x-1,y) = r7
'       if_z            wrbyte :t, r3                   ' DEBUG
        
                        add    r2, #2
'                       add    r3, #2                   ' DEBUG
                        rdbyte :cell_right, r2          wz
        if_z            wrbyte r7, r2                      ' aimap(x+1,y) = r7
'       if_z            wrbyte :t, r3                   ' DEBUG

                        add    r2, #MAP_WIDTH-1
'                       add    r3, #MAP_WIDTH-1         ' DEBUG
                        rdbyte :cell_down, r2           wz
        if_z            wrbyte r7, r2                      ' aimap(x,y+1) = r7
'       if_z            wrbyte :t, r3                   ' DEBUG

                        sub    r2, #MAP_WIDTH           ' move r2 back to center.
'                       sub    r3, #MAP_WIDTH           ' DEBUG

                        cmp    :cell_up, #254           wz
        if_z            mov    :lee_dir, #KEY_DOWN
        if_z            jmp    #:lee_done
                        cmp    :cell_down, #254         wz
        if_z            mov    :lee_dir, #KEY_UP
        if_z            jmp    #:lee_done
                        cmp    :cell_left, #254         wz
        if_z            mov    :lee_dir, #KEY_RIGHT
        if_z            jmp    #:lee_done
                        cmp    :cell_right, #254 wz
        if_z            mov    :lee_dir, #KEY_LEFT
        if_z            jmp    #:lee_done
                                      
:lee_growcontinue1      
                        add    r2, #1
'                       add    r3, #1                   ' DEBUG
                        djnz   r4, #:lee_growloop

' end inner loop //////////////////////////////////////////////////////////////

                        add    r6, #1
                        add    r7, #1
                        djnz   :safe, #:lee_growloop2
...
:cell_left              long                            $0
:cell_right             long                            $0
:controllerptr          long                            _memstart+@g_controllerptr
:playerptr              long                            _memstart+@g_playerptr
:aibuf                  long                            _memstart+@g_aibuf
:lmap                   long                            _memstart+@g_lmap
:fmap                   long                            _memstart+@g_fmap
...
:player_data            long                    1, 16, 16, 0, DIR_RIGHT, 1, 0, 0, 256 << 16, 2, 0, $00000020, 0, 0, 0, 0
                        ...
                        long                    0, 80, 80-32, 0, DIR_RIGHT, 1, 0, 0, 256 << 16, 2, 0, $00000000, 0, 0, 0, 1
                        long                    0, 144, 112+32, 0, DIR_LEFT, 1, 0, 0, 256 << 16, 2, 0, $00000040, 0, 0, 0, 1         
                        long                    0, 144+32, 80, 0, DIR_DOWN, 1, 0, 0, 256 << 16, 2, 0, $00000080, 0, 0, 0, 1
                        long                    0, 80-32, 112, 0, DIR_UP, 1, 0, 0, 256 << 16, 2, 0, $000000c0, 0, 0, 0, 1
:player_data_end
}

{
' 7 AI players 1 human (controller 0)
:player_data            long                    1, 16, 16, 0, DIR_RIGHT, 1, 0, 0, 256 << 16, 2, 0, $00000020, 0, 0, 0, 0
                        long                    1, 208, 176, 0, DIR_LEFT, 1, 0, 0, 256 << 16, 2, 0, $00000060, 0, 0, 0, 1            
                        long                    1, 208, 16, 0, DIR_DOWN, 1, 0, 0, 256 << 16, 2, 0, $000000a0, 0, 0, 0, 1
                        long                    1, 16, 176, 0, DIR_UP, 1, 0, 0, 256 << 16, 2, 0, $000000e0, 0, 0, 0, 1
                        
                        long                    1, 80, 80-32, 0, DIR_RIGHT, 1, 0, 0, 256 << 16, 2, 0, $00000000, 0, 0, 0, 1
                        long                    1, 144, 112+32, 0, DIR_LEFT, 1, 0, 0, 256 << 16, 2, 0, $00000040, 0, 0, 0, 1         
                        long                    1, 144+32, 80, 0, DIR_DOWN, 1, 0, 0, 256 << 16, 2, 0, $00000080, 0, 0, 0, 1
                        long                    1, 80-32, 112, 0, DIR_UP, 1, 0, 0, 256 << 16, 2, 0, $000000c0, 0, 0, 0, 1
:player_data_end
}
...
:player_data            long                    1, 16, 16, 0, DIR_RIGHT, 6, 0, 0, 384 << 16, 10, 0, $00000020, 0, WEAPON_GLOVE, 0, 0
                        ...
                        long                    0, 80, 80-32, 0, DIR_RIGHT, 6, 0, 0, 256 << 16, 10, 0, $00000000, WEAPON_GLOVE, 0, 0, 0
                        long                    0, 144, 112+32, 0, DIR_LEFT, 6, 0, 0, 256 << 16, 10, 0, $00000040, WEAPON_GLOVE, 0, 0, 0
                        long                    0, 144+32, 80, 0, DIR_DOWN, 6, 0, 0, 256 << 16, 10, 0, $00000080, WEAPON_GLOVE, 0, 0, 0
                        long                    0, 80-32, 112, 0, DIR_UP, 6, 0, 0, 256 << 16, 10, 0, $000000c0, WEAPON_GLOVE, 0, 0, 0
:player_data_end
...
' r1: 0...n (player index)
        ...
        if_nz           rdlong :button, r3              ' AI == 0
        if_z            wrlong :button, r3              ' AI >= 1
                        add    r3, #4
                        rdlong :buttonold, r3
        ...
        if_z            jmp    #:cant_punch                        
                        ...
                        test   :buttonold, #KEY_B wc    ' Carry = old KEY_B (0 -> NC)
                        test   :button, #KEY_B  wz      ' Not Zero = KEY_B (1 -> NZ)
                        mov    r2, #16
...
:ittr                   
        ...
        if_c            jmp    #:keyvert
                        ...
                        test   :button, #KEY_LEFT wz
        if_nz           mov    r3, #DIR_LEFT
                        test   :button, #KEY_RIGHT wz
        if_nz           mov    r3, #DIR_RIGHT
:keyvert
                        test   :button, #KEY_UP wz
        if_nz           mov    r3, #DIR_UP
                        test   :button, #KEY_DOWN wz
        if_nz           mov    r3, #DIR_DOWN

        if_c            test   :button, #KEY_LEFT wz
        if_nz_and_c     mov    r3, #DIR_LEFT
        if_c            test   :button, #KEY_RIGHT wz
...
:end_ittr
                        ...
                        cmp    :tc, #0          wz
        if_nz           cmp    :tc, #255        wz      ' ignore predicted flame.
        ...
        if_z            jmpret :itemcollect_ret, #:itemcollect
                        ...
                        mov    r5, r0
                        add    r5, #PLAYER_BUTTONOLD*4
                        wrlong :button, r5      ' backup button state

                        test   :button, #KEY_A  wz      ' nz = A button state
                        test   :buttonold, #KEY_A wc    ' c = previous A button state
...
:drop_bomb
        ...
        if_z            jmp   #:dontdrop
                        ...
                        mov    :tmp0, #SFX_DROPBOMB ' Drop bomb SFX
                        wrlong :tmp0, :sndreq
...
:itemcollect            
        ...
        if_nz           jmp    #:itemcollect_ret
                        ...
                        mov    r3, #SFX_ITEMCOLLECT     ' Item Collect SFX
                        wrlong r3, :sndreq
...
:punch
        ...
        if_nz           jmp    #:skip_punchbomb
                        ...
                        ' Bomb present let's punch this thing
                        mov    r2, #SFX_PUNCHBOMB                       ' Punch bomb SFX
                        wrlong r2, :sndreq
...
:skip
                        ...
                        mov    r2, :frame
                        shr    r2, #5
                        and    r2, #3                   ' 0,1,2,3 -> 0,1,0,2

                        cmp    :t1, #1          wz
        if_nz           mov    r2, #0                   ' Dead/Death - no movement
        ...
        if_z            mov    :t2, #8
                        ...
                        sub    :draw_sprite_h, :t2
                        add    r1, :t2

                        cmp    :x, #256-16      wc
        if_c            jmpret :draw_sprite_ret, #:draw_sprite
                        add    :x, #64
