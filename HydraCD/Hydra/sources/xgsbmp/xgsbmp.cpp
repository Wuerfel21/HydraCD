...
int main(int argc, char* argv[])
{
	...
	else if(!strcmpi(tmp_arg, "-colorxor")) sscanf(ext_arg,"%lx",&g_colorxor);
	...
	if(file_arg<2) {
					...
					printf("\t-op:copy \tDirect copy <Source> -> <Dest>\n");
					printf("\t-hydra \tXGS Hydra color output\n");
					printf("\t-phase \tXGS Hydra phase/luma offset (default 02)\n");
					printf("\t-colorxor \tXGS Hydra color xor operation (default 00)\n");
					...
					printf("\t-bias_red:f \tSpecify Red Bias in color matching e.g. -bias_red:0.4 (default 1.0) \n");
					printf("\t-bias_green:f \tSpecify Green Bias in color matching e.g. -bias_green:0.7 (default 1.0)\n");
					printf("\t-bias_blue:f \tSpecify Blue Bias in color matching e.g. -bias_blue:1.0 (default 1.0)\n");
					return 0;
					}
	// File Operation
	if(g_flag_op)
	{
	if(!strcmpi(g_op_arg,"copy"))
	{
	if(file_arg<2) {
						printf("\n\tUsage: XGSBMP <source.*> <dest.*> <-op:opcode>\n\n");
						return 0;
						}
	setlabel(g_source); // label is the name of the source file.
	convertsrcXGSCOMBINE(g_source, "", g_dest);
	}
	else {
		if(file_arg<3) {
...
void output_file_hexRAW(int x)
{
	...
	if(g_mode == XGS_HYDRA)
		...
		fwrite(&val8, 1, 1, g_output_file_fptr);	
		}
	else {
	if(g_output_file_hex_p<g_output_file_hex_limit)
	fwrite(&val16, 2, 1, g_output_file_fptr);
...
void convertsrcXGSCOMBINE(char *src1, char *src2, char *dest)
{
	...
	else if(!strcmpi(g_op_arg, "copy")) opcode = OPCODE_COPY;
	...
	if(!g_flag_bin)
		...
		if(g_mode==XGS_HYDRA)
			...
			fprintf(g_output_file_fptr,"RETURN @_%s\r\n\r\n",g_output_file_label);
			fprintf(g_output_file_fptr,"DAT\r\n");
			fprintf(g_output_file_fptr,"' Data Type: RAW Data\r\n");
			fprintf(g_output_file_fptr,"' Size: %ld Bytes\r\n", max_length);
			fprintf(g_output_file_fptr,"' Range: %lX -> %lX\r\n", g_output_file_hex_org, g_output_file_hex_org+max_length);
			}
		else {
