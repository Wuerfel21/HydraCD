...
/*
MAP2SPIN.CPP - Converts bitmaps in 8-24 bit color into spin tile code plus palettes
...
Usage MAP2SPIN.exe tilemapinputfile bitmapinputfile [flags] > outputfile
...
// mappy tile sorting struct used to hold a reference count of how many times a bitmap tile is referenced
// as well as the 0 based index it was inserted into the table with, basically this table along with the algorithm
// performs an insertion sort along with a mapping sort from mappy indices to 0 based hydra hel indices for my tile
// engine
typedef struct MAPPY_TILE_SORT_TAG
    {
    int refcount; // reference count
    int index;    // index

    } MAPPY_TILE_SORT, *MAPPY_TILE_SORT_PTR;
...
char bitmapinputfilename[256]; // input bitmap file to converter
char bitmapinputfileroot[256]; // input file root without extention

char tilemapinputfilename[256]; // input "mappy" tile map file to converter
char tilemapinputfileroot[256]; // input file root without extention
...
int num_mappy_sorted_tiles = 0;
MAPPY_TILE_SORT tiles_extracted_count[256];     // this array holds a reference count of the number of times
                                                // a tile is used in the mappy map, and the insertion index of it
                                                // this is used to convert the mappy map indices 0..255 where the indices
                                                // might be sparse into a linear index array, so that only the bitmaps that 
                                                // are used are converted to spin, and the mapping from mappy map to hydra hel
                                                // map are consistent
...
printf("\n\n' Extracted from file \"%s\" at tile=(%d, %d), size=(%d, %d), palette index=(%d)", bitmapinputfilename, tx, ty, width, height, num_tileset_palettes-1);
printf("\n%s_tx%d_ty%d_bitmap    LONG", bitmapinputfileroot,tx,ty);
for (py = 0; py < height; py++)
    {
    printf("\n              LONG ");
...
if (cmd_M)
    ...
    // loop thru image and output spin code
    printf("\n\n' Extracted from file \"%s\" at tile=(%d, %d), size=(%d, %d), palette index=(%d)", bitmapinputfilename, tx, ty, width, height, num_tileset_palettes-1);
    printf("\n%s_tx%d_ty%d_mask", bitmapinputfileroot,tx,ty);
    for (py = 0; py < height; py++)
        {
        printf("\n              LONG ");
...
if (cmd_V) {
    ...
    for (int row = 0; row < map->height; row++)
        ...
        for (int col = 0; col < map->width; col++)
            {
            printf("%2d, ", map->buffer[row*map->width + col]);
            } // end for col
...
// assume first parm after .exe is the tilemapinputfile, good place for more error handling
strcpy(tilemapinputfilename, argv[1]);

// assume second parm after .exe is the bitmapinputfile, good place for more error handling
strcpy(bitmapinputfilename, argv[2]);

// now parse thru remaining command line parms and set all command line globals based on them
for (int cmd_index = 3; cmd_index < argc; cmd_index++)
    {
    // what flag is this?
...
strcpy(bitmapinputfileroot, bitmapinputfilename);
for (int i=0; i < strlen(bitmapinputfileroot); i++)
    if (bitmapinputfileroot[i]=='.') { bitmapinputfileroot[i] = 0; break; }

// now shorten name to no longer than 12 characters since it has to be used in spin var names and there is a 30 char limit
if (strlen(bitmapinputfileroot) >= 12)
   bitmapinputfileroot[12] = 0;

// extract only root of filename
strcpy(tilemapinputfileroot, tilemapinputfilename);
for (int i=0; i < strlen(tilemapinputfileroot); i++)
    if (tilemapinputfileroot[i]=='.') { tilemapinputfileroot[i] = 0; break; }

// now shorten name to no longer than 12 characters since it has to be used in spin var names and there is a 30 char limit
if (strlen(tilemapinputfileroot) >= 12)
   tilemapinputfileroot[12] = 0;
...
// load the mappy map in
Load_Mappy_Map(argv[1], &tile_map);

// now scan the table and sort the bitmap indices and create reference counts and insertion
for (py=0; py < tile_map.height; py++)
    {
    for (px=0; px < tile_map.width; px++)
        {
        // extract bitmap index
        int bitmap_index = tile_map.buffer[px + tile_map.width*py];
        
        // insert bitmap index into sort table, test for first time?
        if (tiles_extracted_count[bitmap_index].refcount == 0)
           {
           // insert this bitmap reference, set the index
           tiles_extracted_count[bitmap_index].refcount++; 
           tiles_extracted_count[bitmap_index].index = num_mappy_sorted_tiles;
           
           // increment the index
           num_mappy_sorted_tiles++;
            
           } // end if
        else // already in table simply increment reference count
          tiles_extracted_count[bitmap_index].refcount++;
...
#if 0
// print tile sort table
printf("\nTotal number of bitmaps referenced in tilemap is %d", num_mappy_sorted_tiles);
for (index=0; index < 256; index++)
  {
  printf("\nTable entry[%d]: bitmap index %d, occurs %d times", index, tiles_extracted_count[index].index, tiles_extracted_count[index].refcount);
  } // end for
#endif


// first print out hydra compliant tile map, then extract all the bitmaps and palettes, the tile map is abstract, so we can do that processing
// after the tile map, all we need to make sure is of the bitmap indices used in the final tile map and the palette indices which is what the construction
// of the tiles_extracted_count[] table helps construct

printf("\n\n' Tilemap based on file \"%s\", dimensions %dx%d", tilemapinputfilename,tile_map.width, tile_map.height);
printf("\n' %d bitmap tiles extracted from bitmap tile set \"%s\"", num_mappy_sorted_tiles,bitmapinputfilename );
printf("\n%s_tilemap    WORD", tilemapinputfileroot);              
...
for (py=0; py < tile_map.height; py++)
    ...
    for (px=0; px < tile_map.width; px++)
        ...
        // extract the tile index and palette index, since there is 1 palette for every unique tile in this tool, the bitmap index = tile index!
        int tile_index    = tiles_extracted_count[bitmap_index].index;
        int palette_index = tiles_extracted_count[bitmap_index].index;

        // print SPIN code out!
        if (px==0)
           printf("\n              WORD      $%02X_%02X", palette_index, tile_index);
        else    
           printf(", $%02X_%02X", palette_index, tile_index);           

        // print some helper text
        if (px==tile_map.width-1)
           printf(" ' Row %d", py);
...
// print header for tile bitmaps
printf("\n_tile_bitmaps    LONG");

// next load in bitmaps refered to by tilemap
if (!Load_Bitmap_File2(&bitmap, bitmapinputfilename))
   { printf("\nFile open error!"); exit(0); }


// ok at this point, we are almost done, we have the tilemap printed the tilemap sort table built, so we just need to extract all the bitmaps from the 
// tile bitmap template which MUST be in the same form as the tile map was made in with the mappy tool, best idea to use the EXACT same bitmap file
// the algorithm will simply walk/scan the original mappy map, and 
...
for (py=0; py < tile_map.height; py++)
    ...
    for (px=0; px < tile_map.width; px++)
        {
        // extract bitmap index from tile map
        int bitmap_index = tile_map.buffer[px + tile_map.width*py];
     
        // test this tiles refcount, if tile has already been extracted (a -1 tags entry), if not extract and output it
        if (tiles_extracted_count[bitmap_index].refcount > 0)
            {
            // compute tile bitmap's x,y location in template to extract fron
            tx = bitmap_index % cmd_TW;
            ty = bitmap_index / cmd_TW;   

            // and extract bitmap
            Extract_Bitmap(tx,ty,                // tile x,y
                           cmd_W,cmd_H,             // width and height of tile
                           cmd_W+cmd_B,cmd_H+cmd_B, // scan step x and y
                           bitmap_buffer, &bitmap);
                
            Build_Palette_From_Bitmap(tx, ty, cmd_W, cmd_H, bitmap_buffer, 1);

            // tag this tile bitmap as already output, so its not extracted again
            tiles_extracted_count[bitmap_index].refcount = -1;
            } // end if
...
printf("\n\n' %d palettes extracted from file \"%s\" using palette color map %d", num_tileset_palettes, bitmapinputfilename, cmd_P);
printf("\n_tile_palette_map    LONG");
printf("\n%s_palette_map    LONG", bitmapinputfileroot);
