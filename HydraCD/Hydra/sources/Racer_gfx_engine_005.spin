...
'' BUG: for the horizon and bitmap font, the sprites will get messed up(it will render y
''    sprite scanlines in wrong order). Because multiple COGS produce scanlines, if a scanline
''    finishes too slowly and the following scanline gets the sprite quicker, then it will
''    mess up the order that the sprite scanlines will be drawn in.(Note: I have added a temp
''    fix to make road rendering take more time (#:Extra_NOP) to match bg rendering time).  
CON
...
main_loop
                        mov temp_adr, sprite_y_pxl_adr 'grab address for 
                        mov temp_var2, #0 'clear value
                        mov temp_var1, #6 'number of sprites
:clear_sprt             add temp_adr, #4 'move to next address
...
skipoutput
                        ...
                        mov temp_var1, #305  '300
:Extra_NOP
                        djnz temp_var1, #:Extra_NOP
...
:Start_sprite
        ...
        mov temp_adr, sprite_y_pxl_adr 'find current pixel row
