...
PUB start(pingroup, pKBD_Frame) : okay
...
'' and auto-repeat will be set to 15cps with a delay of .5s
  ...
  okay := startx(pingroup, %0_000_100, %01_01000, pKBD_Frame)


PUB startx(pingroup, locks, auto, KBD_Frame) : okay
...
'
newcode
                        ' RGW update a counter to indicate the number of scan frames
                        ' Every time a key is pressed, update
                        ' KBD_Frame as it is used by the RND function in basic
                        ' as a possible seed generator
                        add Frame, #1
...
:same                   call    #receive                'receive byte from keyboard
        ...
        if_nc           jmp     #newcode                'unknown, ignore
                        ...
                        ' RGW move the current value of frame counter to KBD_frame
                        wrlong Frame, _KBD_Frame 
...
' RGW Scan frame count per keypress
_KBD_Frame              res     1
