...
Sound Driver - 11KHz, 16-bit, 9 Channels
...
- Provide an Asm API
- Improve the communication protocol to not use the clumbsy NOTHING_PENDING strategy.
- Write document explaining how the driver works
...
(NOTE: There will be another change to this protocol in a later version.)
...
CON
  ...
  'Approximately 11KHz. Do NOT change this value! The calculation of theta_delta
  ...
  #0, CHDAT_THETA, CHDAT_THETA_DELTA, CHDAT_THETA_CYCLED, CHDAT_SHAPE, CHDAT_STOP_TIME, CHDAT_VOLUME, CHDAT_AMP_ENV, CHDAT_ENV_SEG_DURATION, CHDAT_LFSR, CHDAT_PCM_START, CHDAT_PCM_END, CHDAT_PCM_CURR, CHDAT_VOLUME_DELTA, SIZE_OF_CHDAT
...
VAR
  ...
  long pending_shape_ch7
  long pending_freq_ch7
  long pending_duration_ch7
  long pending_volume_ch7
  long pending_amp_env_ch7
  long pending_pcm_start_ch7
  long pending_pcm_end_ch7

  long pending_shape_ch8
  long pending_freq_ch8
  long pending_duration_ch8
  long pending_volume_ch8
  long pending_amp_env_ch8
  long pending_pcm_start_ch8
  long pending_pcm_end_ch8
...
playing, then the old sound stops and the new sound is played.
   ...
   arg_shape:     The desired shape of the sound. Can be any of the
   ...
   arg_duration:  The amount of time (in 1/SAMPLE_RATE of a second) to play
...
playing, then the old sound stops and the new sound is played.
  ...
  pending_amp_env[offset]   := $FFFF_FFFF
...
loop
                    ...
                    mov     temp,sample_counter
                    and     temp,#%0111                 'Look at the low 3-bits of sample_counter
                    cmp     temp,curr_channel   wz      'Only process requests for one channel each 11KHz sound tick
...
get_old_shape       cmp     0,#(SHAPE_SILENT-1)  wz
        if_e        jmp     #skip_shape_release
                    ...
                    'Handle "release"
                    'If infinite duration, set to first release segment and set stop_time to sample_counter
...
shape_save_stop_time mov    0,sample_counter                 'Set stop_time to now to end the current envelope segment
...
skip_pending_shape
        ...
        if_e        jmp   #skip_pending_freq
                    ...
                    shr   _pending_freq,#2                 'Calculate new theta_delta using a trick to multiply by ($2000/10923 (Approx 11KHz))
                    mov   temp,_pending_freq
                    shr   temp,#1
                    add   _pending_freq,temp               'theta_delta = ($2000*frequency)/(10923 (Approx 11KHz))
...
set_new_duration    mov     0,_pending_duration
                    ...
                    add     _pending_duration,sample_counter   wz  'Set new stop_time
        if_z        add     _pending_duration,#1                   'Ensure _pending_duration+sample_counter doesn't become "never stop" (ie. 0)
...
check_at_stop_time  cmp     0,sample_counter      wz         'Check if at end of envelope segment
...
get_volume          mov     mult_x,0
        ...
        if_e        jmp     #mixer                              'If yes, jump to mixer
                    ...
                    add     volume_delta_ptr,    #SIZE_OF_CHDAT
...
mixer
'                    shl     mixed_sample,#1           'Crank volume high as possible for 9 channels without clipping
                    ...
                    mov     volume_delta_ptr,    #channel_data+CHDAT_VOLUME_DELTA
...
volume_ptr              long    channel_data+CHDAT_VOLUME       'Volume in bits 15..8
...
volume_delta_ptr        long    channel_data+CHDAT_VOLUME_DELTA 'Change in volume per sample

channel_data
                        long    0,0,0,0,0,0,0,0,21,0,0,0,0  'Channel 0
                        long    0,0,0,0,0,0,0,0,21,0,0,0,0  'Channel 1
                        long    0,0,0,0,0,0,0,0,21,0,0,0,0  'Channel 2
                        long    0,0,0,0,0,0,0,0,21,0,0,0,0  'Channel 3
                        long    0,0,0,0,0,0,0,0,21,0,0,0,0  'Channel 4
                        long    0,0,0,0,0,0,0,0,21,0,0,0,0  'Channel 5
                        long    0,0,0,0,0,0,0,0,21,0,0,0,0  'Channel 6
                        long    0,0,0,0,0,0,0,0,21,0,0,0,0  'Channel 7

                        long    0,0,0,0,0,0,0,0,21,0,0,0,0  'Channel 8
'                        long    0,0,0,0,0,0,0,0,21,0,0,0,0  'Channel 9
'                        long    0,0,0,0,0,0,0,0,21,0,0,0,0  'Channel 10
'                        long    0,0,0,0,0,0,0,0,21,0,0,0,0  'Channel 11
end_of_channel_data
...
sample_counter          long    1                  'Increments at approx 11KHz (ie. once per driver iteration)
time_to_resume          long    0                  'Used with WAITCNT to synchronize iterations of main loop to 11KHz
