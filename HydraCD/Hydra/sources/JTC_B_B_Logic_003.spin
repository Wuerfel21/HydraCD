...
'///////////////////////////////////////////////////////////////////////
PUB Get_Mem_Address(player_x_, player_y_, ball_x_, ball_y_,ball_bounce, score_, brick_)
    pl_x_adr:=player_x_
    pl_y_adr:=player_y_
    bll_x_adr:=ball_x_
    bll_y_adr:=ball_y_
...
if_b    jmp #check_pl_ball_ret
        mov temp2, bll_x
        add temp2, #4
        sub temp2, playr_x 'subtract ball_x from paddle_x and divide by 4 to get 0-9
        shr temp2, #2 ' /4
        cmp temp2, #7 wz,wc
        if_a mov temp2, #7
        mov temp1, #:Change_Angle 'create a jump table
        shl temp2, #1 '*2
        add temp1, temp2
        jmp temp1
:Change_Angle
...
:Check_ball
        cmp bll_y_check, #88 wz, wc ' check to see if ball is in range of bricks
if_b    jmp #:Next_Side
        sub bll_y_check, #88 ' find the brick x, y coords
        shr bll_y_check, #3  ' /8
...
Load_Level
        ...
        cmp game_level, #4 wz,wc
        if_a mov game_level, #0 'reset levels after 5
...
ball_angle byte 3,3,1,3,1,0,3,1,1 'upper left
           byte 4,1,1,4,1,0,4,4,1 'upper right
           byte 3,2,2,3,2,0,3,3,2 'lower left
