...
CON
  ...
  ' player  
  PLAYER_START_X   = 0
  PLAYER_START_Y   = SCREEN_TOP+55
  PLAYER_RESPAWN_Y = SCREEN_TOP+8
  PLAYER_NORMAL_Y  = 60
...
VAR
  ...
  ' temps
  long dist_from_top, dist_from_left, offscreen_offset
...
PUB start | i, dx, dy
  ...
  repeat i from 0 to CONSTANT(NUM_WALL_SEGMENTS-1)
    wall_segment_x[i] := CONSTANT(-CAVERN_WIDTH/2)

  player_img_ptr := @player_pix

  'play "drop in" sound effect
  snd.PlaySoundFM(0, snd#SHAPE_SAWTOOTH, DROPIN_FREQ_START, snd#DURATION_INFINITE)
...
'///////////////////////////////////////////////////////////////////////
  ...
  repeat while TRUE
    ...
    if((player_img_ptr <> @player_dead_pix) and (player_y < SCREEN_TOP))
      ...
      if( ((LONG[OFFSCREEN_BUFFER][offscreen_offset] >> ((dist_from_left & $0F) << 1)) & %11) <> CAVERN_COLOR )
        snd.PlaySoundFM(0, snd#SHAPE_NOISE, 200, CONSTANT(snd#SAMPLE_RATE/7))
    ...
    'move walls
    if((player_img_ptr == @player_dead_pix) and (player_y > CONSTANT(SCREEN_TOP+8)))
    ...
    if(top_wall_segment_y - SCREEN_TOP > WALL_SEGMENT_HEIGHT)
      ...
      if(player_img_ptr <> @player_dead_pix)  'player alive
        wall_segment_x[top_wall_segment] := ((?rand & $07) * 10) - CONSTANT(35 + CAVERN_WIDTH/2)
      else                                    'player dead
    ...
    if(player_img_ptr == @player_dead_pix) 'player dead
      ...
      if(player_y < PLAYER_START_Y)
        ...
        if(player_y => CONSTANT(SCREEN_TOP+14) and dropin_freq == 0)
          'start playing "drop in" sound effect
          snd.PlaySoundFM(0, snd#SHAPE_SAWTOOTH, CONSTANT(DROPIN_FREQ_START-50), snd#DURATION_INFINITE)
          dropin_freq := DROPIN_FREQ_START
      else
        'respawn player
        player_x := PLAYER_START_X
...
PUB DrawCavern | i, x1, x2, x3, x4, y1, y2, wall_segment, next_wall_segment, left_width1, left_width2, right_width1, right_width2, right_mid_vertex_x, inner_vertex_y, mid_vertex_y, outer_vertex_y

  ' These don't have to be calculated each iteration.
  ' They can be updated by just subtracting WALL_SEGMENT_HEIGHT.
  ' Note: y3 and y4 are equivilent to y2 and y1 respectively, so they have been omitted
  ...
  '"good enough" initial value approximations
  left_width1  := CONSTANT(SCREEN_WIDTH/4)
  right_width1 := CONSTANT(SCREEN_WIDTH/4)
  mid_vertex_y := CONSTANT(SCREEN_TOP+WALL_SEGMENT_HEIGHT/2)
  ...
  repeat i from 0 to NUM_WALL_SEGMENTS-2
    ...
    ' set up coordinates (coordinates are clockwise from 1 to 4 starting with bottom-left)
    ...
    gr.tri(x1, y1, x2, y2, x3, y2)
    gr.tri(x1, y1, x3, y2, x4, y1)
    ...
    ' set colorwidth for walls
    gr.colorwidth(WALL_COLOR, CONSTANT(PIXEL_SHAPE_ROUND + 0))

    ' precomputations for drawing walls
    inner_vertex_y := mid_vertex_y
    mid_vertex_y   := y1 + (y1 ~> 2) + (y1 ~> 4)
    outer_vertex_y := mid_vertex_y + (y1 ~> 3)

    left_width2  := left_width1
    right_width2 := right_width1
    left_width1  := ((x1 - SCREEN_LEFT)>>1)
    right_width1 := ((SCREEN_RIGHT - x4)>>1)

    ' draw left walls
    gr.plot(SCREEN_LEFT, outer_vertex_y)
    gr.line(x1 - left_width1, mid_vertex_y)    'horizontal-ish outer line
    gr.line(x1, y1)                            'horizontal-ish inner line
    gr.plot(x1 - left_width1, mid_vertex_y)
    gr.line(x2 - left_width2, inner_vertex_y)  'vertical-ish connecting line

    ' draw right walls
    gr.plot(SCREEN_RIGHT, outer_vertex_y)
    gr.line(x4 + right_width1, mid_vertex_y)   'horizontal-ish outer line
    gr.line(x4, y1)                            'horizontal-ish inner line
    gr.plot(x4 + right_width1, mid_vertex_y)
    gr.line(x3 + right_width2, inner_vertex_y) 'vertical-ish connecting line
